...
public class AudioRecorder {
    ...
    public AudioRecorder() {
        mExecutorService = Executors.newCachedThreadPool();     //创建一个可缓存线程池
    }
    ...
    public void startRecord() {
        ...
        if (status == Status.STATUS_STOP) {
            i++;
            createDefaultAudio("test" + i);
        }
        if (status == Status.STATUS_NO_READY || audioRecord == null) {
            throw new IllegalStateException("录音尚未初始化,请检查是否禁止了录音权限~");
        }
        Log.d("AudioRecorder", "===startRecord===" + audioRecord.getState() + status);
        try {
    ...
    public void pauseRecord() {
        Log.d("AudioRecorder", "===pauseRecord===" + status);
    ...
    public void stopRecord() {
        Log.d("AudioRecorder", "===stopRecord===" + status);
     ...
     */
    public void release() {
        Log.d("AudioRecorder", "===release===" + status);
        //假如有暂停录音
        try {
            if (filesName.size() >= 0) {
